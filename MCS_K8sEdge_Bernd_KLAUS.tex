...
According to the description in the introduction before, \ac{DSR} is used as a scientific method to construct the \ac{PoC}. For this purpose two real world exmaples are build, which are then gradually refined. Those exmaples are descripte in detail in the following chapter \hyperref[sec:dsrenv]{environment}. The same \hyperref[sec:dsrusecase]{use-cases} are then deployed onto each of these environments. Afterwards, Tests are then defined and carried out on the basis of the issues listed in section \hyperref[sec:generalchallanges]{genereal challenges}. Based on the results, a preferred environment per property can finally be determined.

\section{Environment}
\label{sec:dsrenv}
\subsection{Generel}
The main goal is to create both of the aimed architectures in a similar environment. The target environment should provide good coverage of the diversity encountered in an edge environment. For that poropse the \ac{PoC} is build across \hyperref{https://www.hetzner.com/cloud}{}{}{Hetzner Cloud} and a local site connected via 4G mobile internet.
...
\begin{table}[ht]
    \begin{center}
        \tcbox[sharp corners, boxsep=2mm, boxrule=0.2mm, colframe=gray!20!gray, colback=white]{
        \begin{tabular}{l|l|l|l|l}
            Node & Location & CPU & IP & Latency \\
            \hline
            Master & Nürnberg, DE & dedicated & AMD64 & x \\
            Edge-1 & Ashburn, US & dedicated & AMD64 & 95ms \\
            Edge-2 & Helsinki, FI &  dedicated & AMD64 & 24ms \\
            Raspberry & Kirchberg, AT & dynamic & ARM32 & 40ms \\
        \end{tabular}}
        \caption{\ac{PoC} nodes table}
        \label{tab:poc-nodes}
    \end{center}
\end{table}
