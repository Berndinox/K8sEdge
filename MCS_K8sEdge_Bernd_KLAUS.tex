...
If no clear decision can be made even after going through the criteria catalogue, or if both environments can realise a target architecture, the recommendation goes in the direction of KubeEdge. Applications can be transferred to the system without any modification. Developers do not have to take any special features into account and control or operation is carried out in the same way as on a conventional \ac{K8s} cluster. Research into future developments also points more in the direction of default K8s architecture such as KubeEdge.
...
\begin{acronym}[XXXXX]
    \acro{Aha}[Aha]{AnsiHTMLadapter}
    \acro{API}[API]{applicationprogramminginterface}
    \acro{ARM}[ARM]{advancedRISCmachines}
    \acro{CI/CD}[CI/CD]{continuousintegration/continuousdelivery}
    \acro{CLI}[CLI]{commandlineinterface}
    \acro{CNCF}[CNCF]{CloudNativeComputingFoundation}
    \acro{CNI}[CNI]{containernetworkinterface}
    \acro{CPU}[CPU]{centralprocessingunit}
    \acro{CRD}[CRD]{costumressourcedefinition}
    \acro{CRI}[CRI]{containerruntimeinterface}
    \acro{DNS}[DNS]{domainnamesystem}
    \acro{DSR}[DSR]{designscienceresearch}
    \acro{FQDN}[FQDN]{fullqualifieddomainname}
    \acro{GUI}[GUI]{graphicaluserinterface}
    \acro{HPA}[HPA]{horizontalpodautoscaler}
    \acro{IoT}[IoT]{internet-of-things}
    \acro{IP}[IP]{internetprocotcol}
    \acro{IT}[IT]{informationtechnology}
    \acro{K8s}[K8s]{Kubernetes}
    \acro{KubeFed}[KubeFed]{Kubernetesclusterfederation}
    \acro{MQTT}[MQTT]{messagequeuingtelemetrytransport}
    \acro{NAT}[NAT]{networkaddresstranslation}
    \acro{OS}[OS]{operatingsystem}
    \acro{PoC}[PoC]{proof-of-concept}
    \acro{PSP}[PSP]{PodSecurityPolicies}
    \acro{QoS}[QoS]{qualityofservice}
    \acro{RAM}[RAM]{randomaccessmemory}
    \acro{RBAC}[RBAC]{rolebaseaccesscontrol}
    \acro{SIG}[SIG]{specialinteresstsgroup}
    \acro{VPN}[VPN]{virtualprivatenetwork}
    \acro{WWW}[WWW]{worldwideweb}
\end{acronym}
